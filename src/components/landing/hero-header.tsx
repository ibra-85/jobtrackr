...
export function HeroHeader() {
  ...
  return (
    <header className="relative z-[3]">
      <nav data-state={menuState && "active"} className="fixed z-[3] w-full px-2 group">
        <div
          className={cn(
            "mx-auto mt-2 max-w-6xl px-6 transition-all duration-300 lg:px-12 border border-transparent",
            isScrolled && "bg-zinc-950/80 max-w-4xl rounded-2xl border-white/10 backdrop-blur-lg lg:px-5",
          )}
        >
            ...
            <div className="absolute inset-0 m-auto hidden size-fit lg:block">
              <ul className="flex gap-8 text-sm">
                {menuItems.map((item, index) => (
                  <li key={index}>
                    <Link href={item.href} className="text-zinc-400 hover:text-white block duration-150">
                      <span>{item.name}</span>
                    </Link>
                  </li>
                ))}
