...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec2 p = fragCoord.xy - r * 0.5;

    for (float i, a; i++ < 9.0; ) {
        // Augmenter légèrement la taille des cercles en réduisant l'influence de la distance
        a = (i * i) / 80.0 - length(p) / (r.y * 1.3);
        ...
        // Une seule couleur claire (blanc/gris clair)
        float intensity = 0.03 / denom * sm * (1.2 + sin(a + i));
        o.rgb += vec3(intensity * 0.8); // Gris clair uniforme
    }

    o = tanh(o);
    // Fond gris foncé avec lignes claires
    fragColor = vec4(0.1, 0.1, 0.1, 1.0) + vec4(o.rgb * 0.6, 0.0);
}
